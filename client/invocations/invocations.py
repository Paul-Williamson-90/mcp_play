...
from client.invocations.constants import (
    REPLACEMENT_PATTERNS,
    STOP_AFTER_ATTEMPT,
    WAIT_FIXED,
)
from client.invocations.prompts import (
    DEFAULT_CHOICE_VALIDATION_PROMPT_TEMPLATE,
    DEFAULT_STRUCTURED_PROMPT_TEMPLATE,
)
from client.invocations.pydantics import Choice, Choices, ValidatorChoice
from client.invocations.dtypes import InvocationCallable
...
class MultiInvocationWithValidation:
    ...
    async def structured_invocation(
        ...
        if len(choices.choices) == 1:
            response = choices.choices[0].choice
    ...
    async def non_structured_invocation(
        ...
        for _ in range(self.choices):
            tasks.append(
                self._invocation_task(
                    non_structured_invocation, llm, prompt, inference_kwargs
                )
            )
        results = await asyncio.gather(*tasks)
        choices = Choices(choices=[Choice(choice=result) for result in results])
        if len(choices.choices) == 1:
            return choices.choices[0].choice
        choice = await self.validate(llm, prompt, choices)
        return choice.choice
